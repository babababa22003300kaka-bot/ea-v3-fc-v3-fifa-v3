...
class SmartMessageManager:
    ...
    async def send_new_active_message(
        ...
        logger.info(
            f"🔵 المستخدم {user_id} دخل من جهاز جديد - Device: {device_info} - Device ID: {device_id}"
        )
        ...
        if len(self.user_devices[user_id]) > 1:
            logger.warning(
                f"⚠️ المستخدم {user_id} يستخدم أجهزة متعددة: {len(self.user_devices[user_id])} أجهزة"
            )
            ...
            if user_id in self.user_active_messages:
                ...
                if old_message.get("message_id") != device_id:
                    logger.info(
                        f"🧽 حذف رسالة قديمة للمستخدم {user_id} بسبب استخدام جهاز جديد"
                    )
                    del self.user_active_messages[user_id]
...
⚠️ تحذير: لديك {remaining} محاولات متبقية"""
            ...
            logger.warning(
                f"محاولة فاشلة من المستخدم {user_id}: {validation['error_type']} - Input: {whatsapp_input}"
            )
...
⚠️ تحذير: لديك {remaining} محاولات متبقية"""
            ...
            logger.warning(
                f"محاولة فاشلة من المستخدم {user_id} لطريقة دفع: {payment_method}"
            )
